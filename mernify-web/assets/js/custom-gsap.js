...
(function ($) {
    ...
    const { desktop, desktop_one, desktop_two, desktop_three, desktop_four, desktop_five, desktop_six, desktop_seven, desktop_eight, desktop_nine } = context.conditions;
    if (document.querySelector(".banner-area")) {
        const isDarkMode = document.body.classList.contains("dark");
        // Scrolled brand word blends into surface so it does not overlap about copy
        const bigtextColor = isDarkMode ? "#FFFFFF" : "#f8fafc";
	...
	if ($('.animated-title').length > 0) {
		...
		let endGradient = chroma.scale(['#ffff', '#ffff', '#ffff', '#ffff', '#ffff']);
		cta.to(chars, {
			duration: 0.5,
			scaleY: 0.6,
			ease: "power1.out",
	...
	// 10. Tesimonial Two child (2) Effect
        ...
        "(min-width: 1199px)": function () {
            ...
            },
            {
                ...
                scrub: 5.5, // 👈 add smooth transition delay
                markers: false
                }
            }
            );
    ...
    nn.add("(min-width: 1199px)", () => {
        gsap.fromTo(".testimonial-two-shape",
            { y: "0%" },
            {
                y: "100%",
                ease: "none",
